...
def test_select_flow_resolves_zotero_pdfs_before_rendering_the_picker(
    monkeypatch, tmp_path
):
    """Guards the wiring, not just the helper.

    Discovery papers arrive with an abstract and no ``pdf_path``, which makes
    every fulltext template unselectable in the picker. Resolving a local PDF
    has to happen on this path, before the page is rendered — testing
    `_unlock_fulltext_templates` alone passes happily while nobody calls it.
    """
    profile = _profile(tmp_path)
    paper = _paper("In My Library")
    monkeypatch.setattr(cli, "_fetch_all", lambda *args: ([paper], []))
    monkeypatch.setattr(relevance, "filter_papers", lambda papers, **kwargs: papers)
    monkeypatch.setattr(cli, "_enrich_doi_papers", lambda papers: papers)
    ...
    def annotate(papers):
        for p in papers:
            p["pdf_path"] = "/library/in-my-library.pdf"
        return len(papers)

    monkeypatch.setattr(cli.zotero, "annotate_pdf_paths", annotate)

    handed_to_picker: list[dict] = []
    monkeypatch.setattr(
        cli.selector,
        "select_papers",
        lambda papers, templates, default: handed_to_picker.extend(papers) or [],
    )

    args = _args()
    args.select = True
    cli._run_profile(
        profile, args, dt.date(2026, 8, 7), dt.date(2026, 8, 8), "codex", "model"
    )

    assert [p.get("pdf_path") for p in handed_to_picker] == [
        "/library/in-my-library.pdf"
    ]
