...
START = "## Contents"
END = "---"
GENERATED_NOTE = "<!-- Generated by scripts/update_toc.py — run it after adding a heading. -->"
...
def build_toc(lines: list[str]) -> list[str]:
    ...
    for text, anchor, children in sections:
        ...
        out += [f'        <li><a href="#{a}">{_esc(t)}</a></li>' for t, a in children]
        out += ["      </ul>", "    </details>", "  </li>"]
    out.append("</ul>")
    return out
...
def pages() -> list[Path]:
    ...
    return [
        ...
        for p in candidates
        if p.is_file()
        and any(line.strip() == START for line in p.read_text(encoding="utf-8").splitlines())
    ]
